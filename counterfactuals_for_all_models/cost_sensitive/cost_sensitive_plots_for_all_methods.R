...
load("counterfactuals_for_all_models/cost_sensitive/cost_sensitive_whatif_results.rda")
load("counterfactuals_for_all_models/cost_sensitive/cost_sensitive_nice_results.rda")
load("counterfactuals_for_all_models/cost_sensitive/cost_sensitive_moc_results.rda")
...
write.csv(test_results, "counterfactuals_for_all_models/cost_sensitive/cost_sensitive_combined_results.csv", row.names = FALSE)
...
ggplot(summary_metrics, aes(x = CE_Method, y = Mean_Values, color = Used_Model, group = Used_Model)) +
  ...
  labs(x = "CE Method", y = "Mean Values", color = "Used Model", title = "Quality Metrics for CE Methods Using Cost-sensitive Model") +
  theme_minimal() +
