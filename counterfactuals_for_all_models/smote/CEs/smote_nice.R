...
smote_nice_ce_xgb <- list()

for (i in 1:nrow(smote_filtered_xgb)) {
  x_interest <- smote_filtered_xgb[i,]
  print(smote_predictor_xgb$predict(x_interest))
  nice_classif <- NICEClassif$new(
    smote_predictor_xgb,
    optimization = "sparsity", # sparsity (default), proximity or plausibility.
    x_nn_correct = FALSE,
...
smote_nice_cfactuals_xgb <- rbindlist(smote_nice_ce_xgb, fill = TRUE)
save(smote_nice_cfactuals_xgb, file = "counterfactuals_for_all_models/smote/CEs/smote_nice_cfactuals_xgb.rda")
