...
over_forecast_xgb2 <- predict(model_over2_xgb, modelData_for_prediction, type = "prob")


over_df_forecast_xgb2 <- data.frame(IDRSSD = IDRSSD_Prediction, Prob_xgb = over_forecast_xgb2) 

over_df_forecast_xgb2 <- over_df_forecast_xgb2 %>%
  mutate(Prediction_xgb = ifelse(Prob_xgb.0 > Prob_xgb.1, 0, 1)) %>%
  mutate(Prob_xgb = ifelse(Prediction_xgb == 0, Prob_xgb.0, Prob_xgb.1))

over_df_forecast_xgb2 <- over_df_forecast_xgb2[-c(2,3)]

over_df_forecast_xgb2$Prediction_xgb <- as.factor(over_df_forecast_xgb2$Prediction_xgb)
...
over_merge_dt_rf_ext_xgb2 <- merge(over_merge_dt_rf_ext2,over_df_forecast_xgb2,by="IDRSSD")


over_df_results2 <- over_merge_dt_rf_ext_xgb2 %>%
